...
import qrcode
import segno
from auctions.fusioncharts import FusionCharts
...
class HomeView(View):
    def get(self, request, *args, **kwargs):
        return render(request, 'auctions/chart.html')
...
#             '8'
#             ]
#         chartLabel = "Price Prediction"
#         chartdata = ['1000', '1200', '1500', '1800', '2000', '2200', '2500', '5555']
#         data ={
#                      "labels":labels,
...
def qrcode(request):
    return render(request, "auctions/qr.html")
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            global otp
            otp = rno
            im = segno.make_qr("OTP :"+str(rno))
            im.save(r"auctions/static/qrimage/sai.svg", scale=2.4)
            return HttpResponseRedirect(reverse('auctions:qr'))
...
def validateOTP(request):
    ...
    if otp1 == str(otp):
        ...
        return render(request, "auctions/login.html", {
            "message": "Invalid otp and/or password."
            })
...
def payment(request):
    if request.method == 'POST':
        ...
        client = razorpay.Client(
            auth=('rzp_test_11oVvfqFtEkl7O','CVga7z1k6XbvVhY4coH2OEcy'))
...
def chartData(request):
    ...
    dataSource['chart'] = {
        "caption": "User Bid History",
            "subCaption": "Previous bids",
            "xAxisName": "No of Bids Placed",
            "yAxisName": "Amount placed (In INR)",
            "numberPrefix": "Rs.",
            "theme": "zune"
        }
    ...
    c=0
    for key in Bid.objects.all():
      data = {}
      c+=1
      data['label'] = str(c)
      data['value'] = str(key.amount)
      list1.append(key.amount)
      dataSource['data'].append(data)
      
    a = round(statistics.stdev(list1)+statistics.mean(list1))

    # Create an object for the Column 2D chart using the FusionCharts class constructor
    column2D = FusionCharts("column2D", "ex1" , "600", "350", "chart-1", "json", dataSource)
    return render(request, 'auctions/chart.html', {'output': column2D.render(),'a':a})